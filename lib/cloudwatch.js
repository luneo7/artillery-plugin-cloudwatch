...
const impl = {
  ...
  buildReportResults: function (instance, testReport) {
    ...
    const dimensions = [
      ...
      {
        Name: 'TEST_NAME',
        Value: instance.config[constants.PLUGIN_TEST_NAME]
      }
    ];
  ...
  CloudWatchPlugin: function (scriptConfig, eventEmitter) {
    ...
    eventEmitter.on('stats', (report) => {
      try {
        ...
        for (let i = 0; i < metricData.length; i += maxMetricDataSize) {
          const data = metricData.slice(i, i + maxMetricDataSize);
...
const api = {
  init: function (scriptConfig, eventEmitter) {
    ...
    return impl.CloudWatchPlugin(scriptConfig, eventEmitter);
  }
};
