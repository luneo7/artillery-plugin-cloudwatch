...
const messages = {
  ...
  pluginParamTestNameRequired: constants.THE + constants.PLUGIN_TEST_NAME + constants.PARAM_REQUIRED,
  pluginParamTestNameMustBeString: constants.THE + constants.PLUGIN_TEST_NAME + constants.PARAM_MUST_BE_STRING,
  pluginParamTestNameMustHaveALengthOfAtLeastOne: constants.THE + constants.PLUGIN_TEST_NAME + constants.PARAM_MUST_HAVE_LENGTH_OF_AT_LEAST_ONE,
  pluginParamEnvironmentMustBeString: constants.THE + constants.PLUGIN_ENVIRONMENT + constants.PARAM_MUST_BE_STRING,
  pluginParamEnvironmentHaveALengthOfAtLeastOne: constants.THE + constants.PLUGIN_ENVIRONMENT + constants.PARAM_MUST_HAVE_LENGTH_OF_AT_LEAST_ONE,
};
...
const impl = {
  validateConfig: function (scriptConfig) {
    ...
    } else if (!(typeof scriptConfig.plugins[constants.PLUGIN_NAME][constants.PLUGIN_PARAM_NAMESPACE] === 'string' || scriptConfig.plugins[constants.PLUGIN_NAME][constants.PLUGIN_PARAM_NAMESPACE] instanceof String)) {
    ...
    // Validate TEST_NAME
    if (!(constants.PLUGIN_TEST_NAME in scriptConfig.plugins[constants.PLUGIN_NAME])) {
      throw new Error(messages.pluginParamTestNameRequired);
    } else if (!(typeof scriptConfig.plugins[constants.PLUGIN_NAME][constants.PLUGIN_TEST_NAME] === 'string' || scriptConfig.plugins[constants.PLUGIN_NAME][constants.PLUGIN_TEST_NAME] instanceof String)) {
      throw new Error(messages.pluginParamTestNameMustBeString);
    } else if (scriptConfig.plugins[constants.PLUGIN_NAME][constants.PLUGIN_TEST_NAME].length === 0) {
      throw new Error(messages.pluginParamTestNameMustHaveALengthOfAtLeastOne);
    }

    if ((constants.PLUGIN_ENVIRONMENT in scriptConfig.plugins[constants.PLUGIN_NAME])) {
      if (!(typeof scriptConfig.plugins[constants.PLUGIN_NAME][constants.PLUGIN_ENVIRONMENT] === 'string' || scriptConfig.plugins[constants.PLUGIN_NAME][constants.PLUGIN_ENVIRONMENT] instanceof String)) {
        throw new Error(messages.pluginParamEnvironmentMustBeString);
      } else if (scriptConfig.plugins[constants.PLUGIN_NAME][constants.PLUGIN_ENVIRONMENT].length === 0) {
  ...
  buildCountMetrics: function (instance, testReport) {
    const date = new Date(Math.max.apply(null, testReport._requestTimestamps));
    const timestamp = date instanceof Date && !isNaN(date) ? date : new Date();
  ...
  CloudWatchPlugin: function (scriptConfig, eventEmitter) {
    ...
    eventEmitter.on('stats', (report) => {
      try {
        const latencyMetrics = impl.buildReportResults(self, report);
        const countMetrics = impl.buildCountMetrics(self, report);
        const errorMetrics = impl.buildReportErrors(self, report);

        const metricData = [].concat(latencyMetrics, countMetrics, errorMetrics);
